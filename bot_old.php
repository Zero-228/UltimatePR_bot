                ...
                if ($admins) {
                    foreach ($admins as $administrator) {
                        if (!$administrator->user->is_bot) {
                            ...
                            if ($checkUserInChanel == 'user_not_added' && $bot->chatId() != BOT_ID) {
                                addUserInChanel([
            ...
            if ($bot->chatId() != BOT_ID && $bot->chatId() != $bot->userId()) {
                ...
                if ($settings['subscription'] == 'on') {
                    if ($role == 'user' && $isBot == false) {
                        ...
                        $subsKey = InlineKeyboardMarkup::make();
                        $i = 0;
                        $e = 0;
                        foreach ($groups as $group) {
                            $i++;
                            $groupId = $group['chanelTo'];
                            $check = checkUserInChanel($bot->userId(), $groupId);
                            $btn = msg('subs_btn', $lang).strval($i);
                            if ($check === 'user_not_added') {
                                $e++;
                            } elseif (is_array($check)) {
                                $btn .= "✅";
                            }
                            $subsKey->addRow(InlineKeyboardButton::make($btn, url: "https://t.me/".getChanelUsername($groupId)));
                        }
                        if ($e>0) {
                            $username = getUsername($bot->userId());
                            $bot->deleteMessage($chatId, $bot->messageId());
                            superUpdater('chanel_log', 'status', 'deleted', 'messageId', $bot->messageId());
                            $bot->sendMessage(chat_id: $chatId, text: msg('subs_msg', $lang, ['{username}' => $username]), reply_markup: $subsKey);
                        }
                    }
                }
                if (str_contains($text, "@setup")) {
                    if ($role == 'admin' || $role == 'creator' || $bot->userId() == 1087968824) {
                        ...
                        $command = $parts[0];
                        $groupUsername = isset($parts[1]) ? $parts[1] : '';
                        $timer = isset($parts[2]) ? $parts[2] : '';
                        $bot->deleteMessage($chatId, $bot->messageId());
                        superUpdater('chanel_log', 'status', 'deleted', 'messageId', $bot->messageId());
